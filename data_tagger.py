    ...
    if itemType in DATATYPES:
        with DBase() as cur:
            ...
            # show all tagless items
                ...
                cur.execute("SELECT * FROM {table} WHERE id NOT IN(SELECT {itemType}_id FROM tags WHERE {itemType}_id IS NOT NULL);".format(table, itemType))
            else:
                cur.execute("SELECT {table}.id, name, content FROM tags INNER JOIN tags ON {table}.id = tags.{itemType}_id WHERE tags.tag=(?);".format(table, itemType), (tag,))
...
def index():
    ...
    if request.args:
        ...
        if "tag" in tags.keys():
            files = get_items_by_tag(tags["tag"], 'file') 
            bookmarks = get_items_by_tag(tags["tag"], "bookmark")
            notes = get_items_by_tag(tags["tag"], "note")
            #bookmarks = show_bookmarks_by_tag(tags["tag"])
            # When you delete an item, so the view goes back to the tag you had picked
            tag = tags["tag"]


    return render_template('index.html', tagsCounted=tagsCounted, files = files, bookmarks = bookmarks, notes = notes, tag = tag)
...
if __name__ == '__main__':
    create_base()
    app.run(debug=True, host='192.168.1.120', port=4000)
